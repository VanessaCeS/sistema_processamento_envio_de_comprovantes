...
from auxiliares import alterar_nome_pdf, data_corrente_formatada, deletar_arquivos_pdf, leitura_pdf, remover_acentos, verificar_tipo_arquivo

dados = get_dados_ficha_cadastral(search_xml)

def dividir_e_renomear_pdf_contra_cheque(arquivos, mes_referencia):
    erros = []
    for arquivo in arquivos:
        with open(arquivo, 'rb') as arquivo_pdf:
            leitor_pdf = PdfReader(arquivo_pdf)
            for page_num in range(len(leitor_pdf.pages)):
                leitor_pdf, page_num,  nome, documento, arq_txt = leitura_pdf("contra_cheque", leitor_pdf, page_num)
                if verificar_tipo_arquivo(arq_txt, 'RECIBO DE PAGAMENTO FOLHA MENSAL'):
                    ...
                    erros.append(f"{arquivo} não é do tipo CONTRA CHEQUE")
    deletar_arquivos_pdf()       
    return erros, "#ff0000" if erros else ["Contra cheques anexadas com sucesso!"], "#32965D" 
