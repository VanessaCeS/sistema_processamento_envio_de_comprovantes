...
from auxiliares import alterar_nome_pdf, deletar_arquivos_pdf, leitura_pdf, verificar_tipo_arquivo
  ...
  for arquivo in arquivos:
    with open(arquivo, 'rb') as arquivo_pdf:
      ...
      for page_num in range(len(leitor_pdf.pages)):
        leitor_pdf, page_num, nome, agencia, conta_corrente, arq_txt = leitura_pdf("comprovante", leitor_pdf, page_num)
        if verificar_tipo_arquivo(arq_txt, 'BANCO ITAÚ - COMPROVANTE DE TRANSFERÊNCIA'):
          ...
          erros.append(f"{arquivo} não é do tipo COMPROVANTE")
    deletar_arquivos_pdf()

    return erros, "#ff0000" if erros else ["Comprovantes anexados com sucesso!"], "#32965D"
