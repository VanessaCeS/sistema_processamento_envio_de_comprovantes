...
class ArcherInstance:
    ...
    def get_field_options(self, field_name):
        ...
        try:
            ...
            for value_id in data:
                self.fields[field_name][value_id["RequestedObject"]["Name"]] = value_id[
                    "RequestedObject"
                ]["Id"]
    ...
    def create_sub_record(self, fields_json, subform_name, record_id=None):
        ...
        for key in fields_json.keys():
            ...
            if self.subforms_json_by_sf_name[subform_name][current_id]["Type"] == 9:
                if not isinstance(fields_json[key], list):
                    fields_json[key] = [fields_json[key]]
                list_ids = []
                for id_v in fields_json[key]:
                    if isinstance(id_v, str):
                        if id_v.isnumeric():
                            id_v = int(id_v)
                    list_ids.append({"ContentID": id_v})
                current_json["Value"] = list_ids
            elif self.subforms_json_by_sf_name[subform_name][current_id]["Type"] == 4:
                if not isinstance(fields_json[key], list):
                    fields_json[key] = [fields_json[key]]
                for id_v in fields_json[key]:
                    if isinstance(id_v, str):
                ...
                current_json["Value"] = {"ValuesListIds": fields_json[key]}
            else:
